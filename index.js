...
var produk = (module.exports = {
          ...
          switch (message.type) {
            ...
            case "image":
              ...
              if (
                ...
                let sendImage = await produk.sendImage(
                  message,
                  event.replyToken,
                  event
                );
                if (itemAdmin.isAdmin == true) {
                  ...
                  return client.pushMessage(itemAdmin.userId, [
                    echoMessageId,
                    sendImage,
                    cartData,
                  ]);
            ...
            case "location":
              let echoLocation = await produk.handleLocation(
                message,
                event.replyToken,
                event
              );
  ...
  handleLocation: async function (message, replyToken, event) {
    ...
    let getDestinasi = await axios.get(
      `https://backend-skripsi.herokuapp.com/listKota/${getKota}`
    );
    let getBerat = await axios.get(
    ...
    if (cekDestinasi == null) {
      var ongkir = 0;
      var statusOngkir =
        "Mohon maaf ongkir tidak tersedia, mohon gunakan alamat lain di Indonesia";
    } else {
      ...
      let soko = {
        ...
        body: {
          ...
          contents: [
            ...
            {
              type: "text",
              text: "Berat: " + item.berat.toString(),
              flex: 0,
              margin: "md",
              size: "sm",
              color: "#AAAAAA",
  ...
  handleApprovePayment: async function (event, query) {
    let updatePesanan = await axios.put(
      `https://backend-skripsi.herokuapp.com/pesanan/${query}`,
      {
        adminBayar: true,
      }
    );
    let userToNotify = await axios.get(
      `https://backend-skripsi.herokuapp.com/pesanan/bynomorbayar/${query}`
    );
    let userAdminToNotify = await axios.get(
      `https://backend-skripsi.herokuapp.com/userline/data`
    );
